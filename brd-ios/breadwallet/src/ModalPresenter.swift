...
class ModalPresenter: Subscriber, Trackable {
    ...
    private func addSubscriptions() {
        ...
        Store.subscribe(self, name: .reImportGift(nil)) { [weak self] in
            ...
            if case let .reImportGift(viewModel) = trigger {
                ...
                guard let wallet = Currencies.shared.walletState(for: "btc")?.currency.wallet else { return assertionFailure() }
                let eventName = self.makeEventName([EventContext.gift.name, Event.redeem.name])
    ...
    private func handleGift(qrCode: QRCode) {
        guard let wallet = Currencies.shared.walletState(for: "btc")?.currency.wallet else { return }
    ...
    private func rootModalViewController(_ type: RootModal) -> UIViewController? {
        ...
        case .receiveLegacy:
            guard let btc = Currencies.shared.walletState(for: "btc")?.currency else { return nil }
            return makeReceiveView(currency: btc, isRequestAmountVisible: false, isBTCLegacy: true)
        case .gift :
            guard let currency = Currencies.shared.walletState(for: "btc")?.currency else { return nil }
        ...
        present { [unowned self] scanResult in
            ...
            case .privateKey:
                ...
                alert.addAction(UIAlertAction(title: "BTC", style: .default, handler: { _ in
                    if let wallet = Currencies.shared.walletState(for: "btc")?.currency.wallet {
                ...
                alert.addAction(UIAlertAction(title: "BCH", style: .default, handler: { _ in
                    if let wallet = Currencies.shared.walletState(for: "bch")?.currency.wallet {
                        self.presentKeyImport(wallet: wallet, scanResult: scanResult)
                    }
                }))
    ...
    func presentMenu() {
        ...
        if let btc = Currencies.shared.walletState(for: "btc")?.currency, let btcWallet = btc.wallet {
        ...
        var btcMenu = MenuItem(title: String(format: S.Settings.currencyPageTitle, Currencies.shared.walletState(for: "btc")?.currency.name ?? "Bitcoin"), subMenu: btcItems, rootNav: menuNav)
        btcMenu.shouldShow = { return !btcItems.isEmpty }
        
        // MARK: Bitcoin Cash Menu
        var bchItems: [MenuItem] = []
        if let bch = Currencies.shared.walletState(for: "bch")?.currency, let bchWallet = bch.wallet {
        ...
        var bchMenu = MenuItem(title: String(format: S.Settings.currencyPageTitle, Currencies.shared.walletState(for: "bch")?.currency.name ?? "Bitcoin Cash"), subMenu: bchItems, rootNav: menuNav)
        bchMenu.shouldShow = { return !bchItems.isEmpty }
        
        // MARK: Ethereum Menu
        var ethItems: [MenuItem] = []
        if let eth = Currencies.shared.walletState(for: "eth")?.currency, let ethWallet = eth.wallet {
        ...
        var ethMenu = MenuItem(title: String(format: S.Settings.currencyPageTitle, Currencies.shared.walletState(for: "eth")?.currency.name ?? "Ethereum"), subMenu: ethItems, rootNav: menuNav)
