...
//    private func fetchWkFee(for amount: Amount, completion: @escaping ((TransferFeeBasis?) -> Void)) {
//        guard let dataStore = dataStore,
//              let wallet = dataStore.coreSystem?.wallet(for: amount.currency),
//              let kvStore = Backend.kvStore,
//              let keyStore = dataStore.keyStore,
//              let address = dataStore.address(for: amount.currency)
//        else {
//            presenter?.presentError(actionResponse: .init(error: SwapErrors.noFees))
//            completion(nil)
//            return
//        }
//
//        let value = dataStore.amountFrom(decimal: amount.tokenValue, currency: amount.currency)
//        let sender = Sender(wallet: wallet, authenticator: keyStore, kvStore: kvStore)
...
//    }
//
//    private func fetchEthFee(for amount: Amount, completion: @escaping ((Decimal?) -> Void)) {
//        let address = dataStore?.address(for: amount.currency)
//
